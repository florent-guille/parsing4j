...
public class EtaBuildNode {
	...
	public String getGraphvizRepr(String prefix) {
		...
		if (isFinal) {
			...
			builder.append("\"" + prefix + this.id + "\" [peripheries=2]");
			if (total > 0) {
				builder.append("\n");
			}
		}
		...
		for (EtaBuildNode target : epsilonTransitions) {
			builder.append("\"" + prefix + this.id + "\" -> \"" + prefix + target.id + "\" [label=\"epsilon\"]");
		...
		for (Entry<EtaSymbol, Set<EtaBuildNode>> entry : transitions.entrySet()) {
			for (EtaBuildNode target : entry.getValue()) {
				builder.append("\"" + prefix + this.id + "\" -> \"" + prefix + target.id + "\" [label=\""
						+ entry.getKey().getPrettyRepr() + "\"]");
				total--;
				if (total > 0) {
					builder.append("\n");
				}
