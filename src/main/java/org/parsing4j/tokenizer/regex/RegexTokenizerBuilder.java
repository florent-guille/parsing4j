...
public class RegexTokenizerBuilder {
	...
	public void addPattern(EtaTerminal terminal, Regex regex, MatchingPolicy policy, Set<String> abovePatterns) {
		this.entries.add(new RegexTokenizerBuilderEntry(terminal, regex, policy, abovePatterns));
	}
	...
	public RegexTokenizer build(EtaTerminal eof) throws Exception {
		...
		for (RegexTokenizerBuilderEntry entry : entries) {
			...
			end.setPattern(new Pair<>(entry.policy(), entry.terminal()));
		}
	...
	public record RegexTokenizerBuilderEntry(EtaTerminal terminal, Regex regex, MatchingPolicy policy,
			Set<String> abovePatterns) {
